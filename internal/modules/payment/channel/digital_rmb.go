...
// 中文：DigitalRMBChannel 定义当前包使用的数据结构或接口。
// English: DigitalRMBChannel defines a data structure or interface used by this package.
type DigitalRMBChannel struct {
	// 中文：AppID 保存当前结构中的配置或数据值。
	// English: AppID stores a configuration or data value for this struct.
	AppID string
	// 中文：MerchantID 保存当前结构中的配置或数据值。
	// English: MerchantID stores a configuration or data value for this struct.
	MerchantID string
	...
	GatewayURL string
	// 中文：WalletID 保存当前结构中的配置或数据值。
	// English: WalletID stores a configuration or data value for this struct.
	WalletID string
...
// 中文：NewDigitalRMBChannel 创建并返回对应组件实例。
// English: NewDigitalRMBChannel creates and returns the corresponding component instance.
func NewDigitalRMBChannel(appID, merchantID, apiKey, gatewayURL, walletID, notifyURL string) *DigitalRMBChannel {
	return &DigitalRMBChannel{
		AppID:      appID,
...
func (c *DigitalRMBChannel) Name() string { return "digital_rmb" }

// 中文：CreatePayment 执行当前包中的对应流程。
// English: CreatePayment executes the corresponding workflow in this package.
func (c *DigitalRMBChannel) CreatePayment(ctx context.Context, outTradeNo, subject string, amount int64, scene, notifyURL, returnURL, openID string) (*PayResult, error) {
	if err := c.validate(); err != nil {
		return nil, err
	}
	endpoint, err := gatewayEndpoint(c.GatewayURL, "/payments")
	if err != nil {
		return nil, fmt.Errorf("digital_rmb: %w", err)
	}
	...
	var resp digitalRMBResponse
	if err := gatewayPostJSON(ctx, c.client, endpoint, payload, &resp); err != nil {
		return nil, fmt.Errorf("digital_rmb create payment: %w", err)
	}
	if !gatewayResponseOK(resp.Success, resp.Code) {
		return nil, fmt.Errorf("digital_rmb create payment: code=%s, message=%s", resp.Code, resp.Message)
	}
...
func (c *DigitalRMBChannel) VerifyCallback(_ context.Context, rawData []byte) (*CallbackResult, error) {
	if err := c.validate(); err != nil {
		return nil, err
	}
	fields, signature, err := gatewaySignedFields(rawData)
	if err != nil {
		return nil, fmt.Errorf("digital_rmb: parse callback: %w", err)
	}
	if !gatewayVerifySignature(c.APIKey, fields, signature) {
		return nil, fmt.Errorf("digital_rmb: signature mismatch")
	}
...
func (c *DigitalRMBChannel) Refund(ctx context.Context, outTradeNo, outRefundNo string, totalAmount, refundAmount int64, reason string) (*RefundResult, error) {
	if err := c.validate(); err != nil {
		return nil, err
	}
	...
	var resp digitalRMBResponse
	if err := gatewayPostJSON(ctx, c.client, endpoint, payload, &resp); err != nil {
		return nil, fmt.Errorf("digital_rmb refund: %w", err)
	}
	if !gatewayResponseOK(resp.Success, resp.Code) {
		return nil, fmt.Errorf("digital_rmb refund: code=%s, message=%s", resp.Code, resp.Message)
	}
...
func (c *DigitalRMBChannel) QueryPayment(ctx context.Context, outTradeNo string) (*CallbackResult, error) {
	if err := c.validate(); err != nil {
		return nil, err
	}
	...
	var resp digitalRMBResponse
	if err := gatewayPostJSON(ctx, c.client, endpoint, payload, &resp); err != nil {
		return nil, fmt.Errorf("digital_rmb query: %w", err)
	}
	if !gatewayResponseOK(resp.Success, resp.Code) {
		return nil, fmt.Errorf("digital_rmb query: code=%s, message=%s", resp.Code, resp.Message)
	}
...
func (c *DigitalRMBChannel) ClosePayment(ctx context.Context, outTradeNo string) error {
	if err := c.validate(); err != nil {
		return err
	}
	if outTradeNo == "" {
		return fmt.Errorf("digital_rmb: out_trade_no is required")
	}
	endpoint, err := gatewayEndpoint(c.GatewayURL, "/payments/close")
	if err != nil {
		return fmt.Errorf("digital_rmb: %w", err)
	}
	...
	var resp digitalRMBResponse
	if err := gatewayPostJSON(ctx, c.client, endpoint, payload, &resp); err != nil {
		return fmt.Errorf("digital_rmb close payment: %w", err)
	}
	if !gatewayResponseOK(resp.Success, resp.Code) {
		return fmt.Errorf("digital_rmb close payment: code=%s, message=%s", resp.Code, resp.Message)
	}
	return nil
}
...
func (c *DigitalRMBChannel) CallbackSuccess() any { return "success" }

// 中文：CallbackFail 执行当前包中的对应流程。
// English: CallbackFail executes the corresponding workflow in this package.
func (c *DigitalRMBChannel) CallbackFail() any { return "fail" }

// 中文：validate 执行当前包中的对应流程。
// English: validate executes the corresponding workflow in this package.
func (c *DigitalRMBChannel) validate() error {
	if c.AppID == "" {
		return fmt.Errorf("digital_rmb: app_id is required")
	}
	if c.MerchantID == "" {
		return fmt.Errorf("digital_rmb: merchant_id is required")
	}
	if c.APIKey == "" {
		return fmt.Errorf("digital_rmb: api_key is required")
	}
	if c.GatewayURL == "" {
		return fmt.Errorf("digital_rmb: gateway_url is required")
	}
	return nil
}

// 中文：digitalRMBResponse 定义当前包使用的数据结构或接口。
// English: digitalRMBResponse defines a data structure or interface used by this package.
type digitalRMBResponse struct {
...
// 中文：digitalRMBPaymentData 定义当前包使用的数据结构或接口。
// English: digitalRMBPaymentData defines a data structure or interface used by this package.
type digitalRMBPaymentData struct {
	// 中文：OutTradeNo 保存当前结构中的配置或数据值。
	// English: OutTradeNo stores a configuration or data value for this struct.
	OutTradeNo string `json:"out_trade_no"`
	// 中文：TradeNo 保存当前结构中的配置或数据值。
...
// 中文：digitalRMBRefundData 定义当前包使用的数据结构或接口。
// English: digitalRMBRefundData defines a data structure or interface used by this package.
type digitalRMBRefundData struct {
	// 中文：RefundNo 保存当前结构中的配置或数据值。
	// English: RefundNo stores a configuration or data value for this struct.
	RefundNo string `json:"refund_no"`
	// 中文：Status 保存当前结构中的配置或数据值。
...
func (r digitalRMBResponse) paymentData() digitalRMBPaymentData {
	data := digitalRMBPaymentData{
...
func (r digitalRMBResponse) refundData() digitalRMBRefundData {
	data := digitalRMBRefundData{RefundNo: r.RefundNo, Status: r.Status}
	if len(r.Data) > 0 {
		_ = json.Unmarshal(r.Data, &data)
	}
	return data
